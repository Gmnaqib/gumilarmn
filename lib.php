...
/**
 * The gumilar block helper functions and callbacks
 *
 ...
 * @return boolean
 */
function block_gumilar_comment_validate($comment_param)
{
    if ($comment_param->commentarea != 'gumilar_data') {
 ...
 * @return array
 */
function block_gumilar_comment_permissions($args)
{
    global $DB, $USER;
    ...
    if ($args->context->contextlevel == CONTEXT_USER && $USER->id != $args->context->instanceid) {
        // Check whether the context owner has a gumilar block in the user's profile.
        $sqlparam = [
            'blockname' => 'gumilar',
            'parentcontextid' => $args->context->id,
            'pagetypepattern' => 'user-profile',
        ];
        // If the gumilar block is not present at the target user's profile,
 ...
 * @return boolean
 */
function block_gumilar_comment_display($comments, $args)
{
    if ($args->commentarea != 'gumilar_data') {
        throw new comment_exception('invalidcommentarea');
    }
    if ($args->itemid != 0) {
        throw new comment_exception('invalidcommentitemid');
